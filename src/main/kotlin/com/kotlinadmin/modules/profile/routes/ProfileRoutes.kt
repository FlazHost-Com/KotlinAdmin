...
fun Application.profileModule() {
    ...
    routing {
        namedGet("admin.v1.profile.index", "/admin/v1/profile") {
            ...
            call.respondView("profile/profile.ftl", mapOf(
                "user" to user.toMap(),
        ...
        namedPut("admin.v1.profile.update", "/admin/v1/profile/update") {
            ...
            if (errors.isNotEmpty()) {
                call.sessions.set(session.withErrors(errors, mapOf(
                    "name" to name, "email" to email, "code" to code, "phone" to phone, "timezone" to timezone
                )))
            ...
            val dto = UpdateProfileDto(
                ...
                phone = phone.ifBlank { null }, timezone = timezone, status = status
            )
