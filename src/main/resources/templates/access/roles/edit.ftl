...
<head>
  ...
  <title>${setting.name!"KotlinAdmin"} - Edit Role</title>
</head>
    ...
    <main class="flex-1 p-6">
      <div class="tw-card p-6 max-w-2xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-semibold" style="color:var(--primary)">Edit Role</h2>
          <a href="/admin/v1/access/role" class="btn btn-light btn-sm">
            <i class="fas fa-arrow-left"></i> Back
          </a>
        </div>
        <form method="POST" action="/admin/v1/access/role/${data.id}/update?_method=PUT&_csrf=${_csrf}">
          <div class="mb-4">
          ...
          <div class="mb-6">
            ...
            <textarea name="description" rows="3"
                      class="form-control <#if (errors.description)??>is-invalid</#if>">${(old.description)!(data.description)!""}</textarea>
